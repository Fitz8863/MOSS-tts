...
class OnnxTtsRuntime(OrtCpuRuntime):
    ...
    def _load_reference_audio(self, reference_audio_path: str | Path) -> np.ndarray:
        # ONNX-only board path: use stdlib WAV loading instead of PyTorch/torchaudio.
        path = Path(reference_audio_path).expanduser().resolve()
        try:
            wav_source = wave.open(str(path), "rb")
        except (wave.Error, EOFError) as exc:
            # Official assets may carry FLAC payloads with a .wav suffix.
            try:
                ...
                wav_source = wave.open(io.BytesIO(converted.stdout), "rb")
            except (FileNotFoundError, subprocess.CalledProcessError) as ffmpeg_exc:
                raise ValueError(
                    f"Reference audio is not a PCM WAV and ffmpeg fallback failed: {path}"
                ) from ffmpeg_exc
        ...
        if channels <= 0 or audio.size % channels:
            raise ValueError(f"Invalid WAV channel layout: channels={channels}, samples={audio.size}")
        ...
        if sample_rate != target_sample_rate and audio.shape[1] > 1:
            target_length = max(1, round(audio.shape[1] * target_sample_rate / sample_rate))
            old_x = np.linspace(0.0, 1.0, audio.shape[1], endpoint=False, dtype=np.float32)
            new_x = np.linspace(0.0, 1.0, target_length, endpoint=False, dtype=np.float32)
            audio = np.vstack([np.interp(new_x, old_x, channel).astype(np.float32) for channel in audio])
