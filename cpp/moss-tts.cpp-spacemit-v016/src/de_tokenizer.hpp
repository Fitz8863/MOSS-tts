#ifndef MOSS_DE_TOKENIZER_HPP
#define MOSS_DE_TOKENIZER_HPP

// Qwen3 byte-level BPE tokenizer (adapted from a proven Qwen2 tokenizer of the
// same BPE family).
//
// Loads vocab + merges from a `tokenizer.gguf` produced by
// scripts/convert_tokenizer.py.
//
// Pre-tokenizer is a hand-written approximation of the Qwen2/Qwen3 regex:
//   (?i:'s|'t|'re|'ve|'m|'ll|'d)
//   |[^\r\n\p{L}\p{N}]?\p{L}+
//   |\p{N}
//   |\s?[^\s\p{L}\p{N}]+[\r\n]*
