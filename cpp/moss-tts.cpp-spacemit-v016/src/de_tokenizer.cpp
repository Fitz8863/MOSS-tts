...
DeTokenizer::DeTokenizer() { build_byte_maps(); }

void DeTokenizer::build_byte_maps() {
    uint32_t cps[256];
...
bool DeTokenizer::load(const ModelLoader& m) {
    ...
    if (tokens_.empty()) {
        MOSS_LOGE("DeTokenizer: tokenizer.tokens missing/empty");
        return false;
    }
    ...
    MOSS_LOGI("DeTokenizer: loaded %zu tokens, %zu merges, %zu special",
              tokens_.size(), merge_rank_.size(), special_.size());
    return true;
}

bool DeTokenizer::load_from_file(const std::string& path) {
...
const std::string& DeTokenizer::token(int32_t id) const {
...
std::vector<std::string> DeTokenizer::pre_tokenize(const std::string& text) const {
...
int DeTokenizer::bpe_pretoken_to_ids(const std::string& pre_tok,
...
std::vector<int32_t> DeTokenizer::encode(const std::string& text) const {
...
std::string DeTokenizer::decode(const std::vector<int32_t>& ids) const {
    std::string bytes;
