...
ggml_threadpool_t g_threadpool = nullptr;
std::string     g_name;
std::once_flag  g_once;

int cpu_thread_count() {
    constexpr int kDefaultThreads = 8;
    const char* env = std::getenv("MOSS_TTS_THREADS");
    if (!env || !*env) return kDefaultThreads;

    char* end = nullptr;
    const long requested = std::strtol(env, &end, 10);
    if (end == env || *end != '\0' || requested < 1 || requested > GGML_MAX_N_THREADS) {
        MOSS_LOGW("backend: ignoring invalid MOSS_TTS_THREADS=%s; using %d",
                  env, kDefaultThreads);
...
void init() {
    ...
    const char* env = std::getenv("MOSS_TTS_BACKEND");
    std::string want = env ? lower(env) : "";

    // 1. Honor MOSS_TTS_BACKEND verbatim if it matches a registered
    // device.
    if (!want.empty()) {
        if (want == "cpu") {
            ...
            if (!g_backend) {
                MOSS_LOGW("backend: MOSS_TTS_BACKEND=%s requested but no "
    ...
    if (g_backend) {
        ...
        // Keep one process-wide CPU threadpool. Without this, ggml creates and
        // destroys a disposable pool for every graph compute. Nano TTS runs
        // many small autoregressive/codec graphs, so that overhead is large on
        // K3 and repeatedly re-runs the SpaceMIT A100 worker binding path.
        if (ggml_backend_is_cpu(g_backend)) {
            const int n_threads = cpu_thread_count();
            ggml_threadpool_params params = ggml_threadpool_params_default(n_threads);
            g_threadpool = ggml_threadpool_new(&params);
            if (g_threadpool) {
                ggml_backend_cpu_set_threadpool(g_backend, g_threadpool);
            } else {
                MOSS_LOGW("backend: failed to create persistent CPU threadpool; "
                          "falling back to per-graph pools");
            }
            ggml_backend_cpu_set_n_threads(g_backend, n_threads);
            MOSS_LOGI("backend: CPU threads=%d, persistent_threadpool=%s",
                      n_threads, g_threadpool ? "yes" : "no");
        }
        ...
        MOSS_LOGI("backend: %s", g_name.c_str());
    } else {
        MOSS_LOGE("backend: failed to initialize any backend");
        g_name = "none";
    }
}
...
bool compute_graph(ggml_cgraph* graph) {
    ...
    if (!ggml_gallocr_alloc_graph(g_gallocr, graph)) {
        MOSS_LOGE("backend: gallocr_alloc_graph failed");
        return false;
    }
...
ggml_backend_buffer_type_t spacemit_weight_buffer_type() {
    ggml_backend_t b = backend();
    if (!b || !ggml_backend_is_cpu(b)) return nullptr;
    ...
    ggml_backend_buffer_type_t* extra = get_extra(dev);
    if (!extra) return nullptr;
    for (size_t i = 0; extra[i] != nullptr; ++i) {
        const char* name = ggml_backend_buft_name(extra[i]);
        if (name && std::strcmp(name, "CPU_RISCV64_SPACEMIT") == 0) {
            return extra[i];
        }
    }
    return nullptr;
...
bool spacemit_weight_buffer_supports_mul_mat(ggml_tensor* weight,
                                              ggml_backend_buffer_type_t buft) {
    ggml_backend_t b = backend();
    if (!b || !ggml_backend_is_cpu(b) || !weight || !buft || weight->buffer) {
        return false;
    }
    ...
    ggml_init_params p {};
    p.mem_size = ggml_tensor_overhead() * 4;
    p.no_alloc = true;
    ggml_context* ctx = ggml_init(p);
    if (!ctx) return false;

    ggml_tensor* input = ggml_new_tensor_2d(ctx, GGML_TYPE_F32, weight->ne[0], 1);
    ggml_tensor* op = ggml_mul_mat(ctx, weight, input);

    // Like llama.cpp's weight_buft_supported(): the extra CPU backend only
    // needs the source tensor's buffer type plus its shape/type for this probe.
    weight->buffer = ggml_backend_buft_alloc_buffer(buft, 0);
    const bool supported = weight->buffer && ggml_backend_dev_supports_op(dev, op);
    if (weight->buffer) ggml_backend_buffer_free(weight->buffer);
...
bool backend_supports_flash_attn() {
    ...
    std::call_once(detect_once, [] {
        const char* env = std::getenv("MOSS_TTS_FLASH_ATTN");
        if (env && (std::string(env) == "0" || lower(env) == "false" || lower(env) == "off")) {
            MOSS_LOGI("backend: flash-attn disabled via MOSS_TTS_FLASH_ATTN=%s", env);
            supported = false;
            return;
        }
        ...
        MOSS_LOGI("backend: flash-attn %s on %s",
