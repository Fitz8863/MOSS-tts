// Closed-loop TTS -> ASR roundtrip (WER-style word-recall) gate.
//
// 1. Load the V1 MossTTSDelay stack and synthesize a known, ASR-friendly phrase
//    to a wav (in-process, via moss::Delay).
...
// 8B Delay backbone each keep a fair amount of static ggml state and would
// fight over compute pools loaded back-to-back; a short-lived child process is
// also how anyone wiring TTS -> ASR will actually do it.
//
// Skips (return 77) unless ALL of these env vars are set:
//   MOSS_TTS_DELAY       backbone gguf            (for tts)
...
int main() {
    ...
    if (!bb || !cd || !tk || !file_ok(cli) || !file_ok(asr)) {
        std::fprintf(stderr,
            "skip: closed-loop needs MOSS_TTS_DELAY, MOSS_TTS_TOKENIZER, "
            "MOSS_DE_TOKENIZER, MOSS_PARAKEET_CLI and MOSS_PARAKEET_MODEL.\n");
        return 77;
    }

    const std::string text = "the quick brown fox jumps over the lazy dog";
    const std::string wav_path = "/tmp/moss_cl.wav";
    const std::string txt_path = "/tmp/moss_cl.txt";

    // 1. TTS -> wav. Pin the seed so the recall threshold doesn't flap between
    //    runs (each unseeded run samples different audio tokens).
    {
        moss::Delay d;
        if (!d.load(bb, cd, tk)) { std::fprintf(stderr, "FAIL: load\n"); return 1; }
        moss::DelayParams p;
        p.seed = 12345;
        std::vector<float> wav;
        int sr = 0;
        if (!d.tts(text, p, &wav, &sr)) { std::fprintf(stderr, "FAIL: tts\n"); return 1; }
