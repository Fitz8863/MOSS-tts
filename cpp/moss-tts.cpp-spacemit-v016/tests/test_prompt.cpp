// Exact input_ids parity for build_generation_prompt vs the upstream python
// (moss_tts_delay/llama_cpp/processor.py:build_generation_prompt). The fixture
// tests/fixtures/prompt.gguf (produced by scripts/gen_delay_reference.py prompt)
// dumps two cases:
...
int main() {
    const char* fx_env = std::getenv("MOSS_FIXTURE_PROMPT");
    std::string fx_path = fx_env ? fx_env : "tests/fixtures/prompt.gguf";
    ...
    if (!tok.load_from_file(tok_path)) {
        std::fprintf(stderr, "prompt: tokenizer load failed: %s\n", tok_path.c_str());
        return 77;  // skip when tokenizer fixture absent
    }

    const int n_vq = (int)ld.get_u32("n_vq");
    if (n_vq != N_VQ) {
        std::fprintf(stderr, "prompt: n_vq mismatch fixture=%d N_VQ=%d\n", n_vq, N_VQ);
        return 1;
    }
    ...
    if (ids_a.empty() || ids_b.empty() || ref.empty()) {
        std::fprintf(stderr, "prompt: missing fixture tensors\n");
        return 1;
    }

    moss::PromptOpts opts;  // defaults render as "None", matching the dump.
    ...
    // Case (a): no reference.
    {
        ...
        std::vector<int32_t> got =
            moss::build_generation_prompt(tok, text, {}, 0, opts, &S);
    ...
    // Case (b): with reference.
    {
        int S = 0;
        std::vector<int32_t> got =
            moss::build_generation_prompt(tok, text, ref, T_ref, opts, &S);
        failures += compare_case("ref", got, S, ids_b, S_b);
    }

    if (failures) {
        std::fprintf(stderr, "prompt: %d case(s) failed\n", failures);
        return 1;
    }
    std::printf("prompt ok: 2 cases exact-match (S_a=%d S_b=%d T_ref=%d)\n",
                S_a, S_b, T_ref);
    return 0;
}
