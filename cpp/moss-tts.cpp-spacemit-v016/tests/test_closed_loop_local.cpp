// Closed-loop TTS -> ASR roundtrip (WER-style word-recall) gate for V2
// MossTTSLocal.
//
// 1. Load the V2 MossTTSLocal stack and synthesize a known, ASR-friendly phrase
//    to a wav (in-process, via moss::Local).
...
// Local backbone each keep a fair amount of static ggml state and would fight
...
//   MOSS_TTS_LOCAL       Local backbone gguf      (for tts)
...
int main() {
    ...
    if (!lc || !cd || !tk || !file_ok(cli) || !file_ok(asr)) {
        std::fprintf(stderr,
            "skip: closed-loop needs MOSS_TTS_LOCAL, MOSS_TTS_TOKENIZER, "
            "MOSS_DE_TOKENIZER, MOSS_PARAKEET_CLI and MOSS_PARAKEET_MODEL.\n");
        return 77;
    }

    const std::string text = "the quick brown fox jumps over the lazy dog";
    const std::string wav_path = "/tmp/moss_cl_local.wav";
    const std::string txt_path = "/tmp/moss_cl_local.txt";

    // 1. TTS -> wav. Pin the seed so the recall threshold doesn't flap between
    //    runs (each unseeded run samples different audio tokens).
    {
        moss::Local m;
        if (!m.load(lc, cd, tk)) { std::fprintf(stderr, "FAIL: load\n"); return 1; }
        moss::LocalParams p;
        p.seed = 12345;
