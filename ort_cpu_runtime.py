...
def _normalize_execution_provider(raw_execution_provider: str | None) -> str:
    ...
    if normalized in {EXECUTION_PROVIDER_SPACEMIT, "SpaceMITExecutionProvider".lower(), "spacemit_ep"}:
        return EXECUTION_PROVIDER_SPACEMIT
    raise ValueError("execution_provider must be one of: cpu, cuda, spacemit")


def _enable_spacemit_ort() -> None:
    # The vendor package monkey-patches the already imported ORT module.  It
    # must be imported before asking ORT for providers or creating sessions.
    try:
        import spacemit_ort  # noqa: F401
    except ImportError as exc:
        raise RuntimeError(
            "SpaceMITExecutionProvider was requested, but spacemit_ort could not be imported. "
            "Use the board's spacemit ONNX Runtime environment (not board_root's generic ORT)."
        ) from exc
    if "+spacemit" not in str(getattr(ort, "__version__", "")):
        raise RuntimeError(
            "SpaceMITExecutionProvider requires the vendor ONNX Runtime build; "
            f"loaded onnxruntime {getattr(ort, '__version__', 'unknown')} from {getattr(ort, '__file__', 'unknown')}"
        )


def _resolve_ort_providers(execution_provider: str, thread_count: int) -> tuple[list[Any], list[dict[str, str]]]:
...
class OrtCpuRuntime:
    def __init__(
        ...
        self.ort_providers, self.ort_provider_options = _resolve_ort_providers(
            self.execution_provider, self.thread_count
        )
        self.actual_session_providers: dict[str, list[str]] = {}
    ...
    def _session(self, path_value: Path) -> ort.InferenceSession:
        ...
        self.actual_session_providers[str(path_value)] = list(session.get_providers())
        ...
        if self.execution_provider == EXECUTION_PROVIDER_SPACEMIT and "SpaceMITExecutionProvider" not in session.get_providers():
            raise RuntimeError(
                "SpaceMITExecutionProvider was requested, but the session did not retain it for "
                f"{path_value}. Actual providers: {session.get_providers()}"
            )
        return session
    ...
    def run_local_greedy_frame(
        ...
        *,
        ...
        session = self.sessions["local_greedy_frame"]
        feed = {
            "global_hidden": global_hidden.astype(np.float32, copy=False),
            "repetition_seen_mask": repetition_seen_mask,
            "repetition_penalty": np.asarray([float(repetition_penalty)], dtype=np.float32),
        }
        # Quantized/community exports are not always input-compatible with the
        # official ONNX export.  ORT rejects extra feed entries, so only pass
        # tensors that the loaded graph declares.
        input_names = {item.name for item in session.get_inputs()}
        outputs = session.run(None, {name: value for name, value in feed.items() if name in input_names})
    ...
    def run_local_fixed_sampled_frame(
        ...
        *,
        ...
        session = self.sessions["local_fixed_sampled_frame"]
        feed = {
            "global_hidden": global_hidden.astype(np.float32, copy=False),
            "repetition_seen_mask": repetition_seen_mask,
        ...
        # ModelScope INT8 removes assistant_random_u from the fixed-sampling
        # graph.  Construct the feed from the graph's declared inputs so both
        # the official and community exports remain usable.
        input_names = {item.name for item in session.get_inputs()}
        outputs = session.run(None, {name: value for name, value in feed.items() if name in input_names})
