...
For custom text input, tokenize with `tokenizer.model` using the same SentencePiece model used by the C++ ONNX runtime, then pass the resulting token ids into `MossOnnxDemoEngine.synthesize`.
